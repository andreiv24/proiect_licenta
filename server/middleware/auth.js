...
const verify = ( req, res, resolve, reject, rights ) => async(err,user)=>{
    if( err || !user ){
        return reject(new ApiError(httpStatus.UNAUTHORIZED,'Neautorizat'));
    }
    req.user = user;
    if (rights.length){
        ...
        if(!permission.granted){
            return reject(new ApiError(httpStatus.FORBIDDEN,"Ne pare rau, nu aveti suficiente drepturi pentru acest lucru"))
        }
